...
contract SBT is IERC4973, ERC165, ERC721Metadata {
    ...
    constructor(string memory _name, string memory _symbol) ERC721Metadata (_name, _symbol) {
        ...
        supportsInterface(bytes4(keccak256("balanceOf(bytea4)")^keccak256("townerOf(bytes4)")^keccak256("burn(bytes4)")));
    }
    ...
    function mint(address to) external virtual {
        ...
        // SBT用にSVGイメージを作成します。
        string memory finalSvg = string(abi.encodePacked(svgPartOne, "2022/7/15", svgPartTwo));
        // SVGのデータをBase64の形式でエンコードする。
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "This is my SBT", "image": "data:image/svg+xml;base64,',
     ...
     * totalSupplyメソッド
     */
    function totalSuuply() external view returns (uint) {
